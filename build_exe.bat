@echo off
rem ===========================================================================
rem  build_exe.bat  --  build JusicRoomPlayer as a single-file Windows EXE
rem ---------------------------------------------------------------------------
...
rem  Output: dist\JusicRoomPlayer <VERSION>.exe
rem ===========================================================================
setlocal
cd /d "%~dp0"
chcp 65001 >nul
...
set "EXE=%DISTDIR%\%APPNAME% %VER%.exe"
echo Version : %VER%
echo Target  : %EXE%
...
rem ---- [4/6] build ----
echo [4/6] Building single-file EXE ... takes a few minutes, please wait
...
if errorlevel 1 goto :err
if not exist "%EXE%" (
    echo [ERROR] Build reported success but the EXE is missing: "%EXE%"
    goto :err
)

rem ---- [5/6] ship license / notices ----
echo [5/6] Copy LICENSE and THIRD_PARTY_NOTICES ...
...
set "SIZE=?"
for %%F in ("%EXE%") do set /a "SIZE=%%~zF / 1048576"
echo.
echo Done in %EMIN%m %ESEC%s :  %EXE%  [%SIZE% MB]
...
echo.
echo Output: dist\%APPNAME% ^<VERSION^>.exe
endlocal
exit /b 0
